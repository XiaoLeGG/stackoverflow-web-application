<template>
  <StatisticsContainer>
    ...
    <template v-slot:introduction>
      <p>This diagram show the top 10 tags or tag combinations with most upvotes.</p>
    </template>
    ...
    <template v-slot:chart>
      <VerticalBarChart v-if="display_chart" ref="chart" title="Tags Upvotes"/>
    </template>
  </StatisticsContainer>
</template>
  ...
  methods: {
    onQuery() {
      ...
      if (this.$refs.number_input.$data.num === 1) {
        axios.get("/api/tag/single-tag/upvote", {
          params: {
        ...
        axios.get("/api/tag/group-tag/upvote", {
          params: {
            from: date_from_string,
            end: date_end_string,
