<template>
  <StatisticsContainer>
    ...
    <template v-slot:introduction>
      <p>This diagram show the top 10 tags or tag combinations that frequently appeared.</p>
    </template>
    ...
    <template v-slot:chart>
      <VerticalBarChart v-if="display_chart" ref="chart" title="Frequent Tags"/>
    </template>
  </StatisticsContainer>
</template>
  ...
  methods: {
    onQuery() {
      ...
      if (this.$refs.number_input.$data.num === 1) {
        axios.get("/api/tag/single-tag/count", {
          params: {
        ...
        axios.get("/api/tag/group-tag/count", {
          params: {
            from: date_from_string,
            end: date_end_string,
